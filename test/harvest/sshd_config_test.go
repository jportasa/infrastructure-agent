...
func TestSshdConfig(t *testing.T) {
	...
	require.NoError(t, os.Mkdir(filepath.Join(tmpDir, "ssh"), 0777))
	tmpfn := filepath.Join(tmpDir, "ssh", "sshd_config")
	err = ioutil.WriteFile(tmpfn, []byte(fakeSshdConfig), 0666)
	require.NoError(t, err)

	os.Setenv("HOST_ETC", tmpDir)
	...
	case req = <-testClient.RequestCh:
		a.Terminate()
	case <-time.After(testhelpers.InventoryDuration(a.Context.Config().SshdConfigRefreshSec)):
		a.Terminate()
	...
	fixture.AssertRequestContainsInventoryDeltas(t, req, []*inventoryapi.RawDelta{
		{
			...
			Diff: map[string]interface{}{
				"PermitRootLogin": map[string]interface{}{
					"id":    "PermitRootLogin",
					"value": "yes",
				},
				"PermitEmptyPasswords": map[string]interface{}{
					"id":    "PermitEmptyPasswords",
					"value": "no",
				},
				"PasswordAuthentication": map[string]interface{}{
					"id":    "PasswordAuthentication",
					"value": "yes",
				},
				"ChallengeResponseAuthentication": map[string]interface{}{
					"id":    "ChallengeResponseAuthentication",
					"value": "no",
				},
			},
		},
