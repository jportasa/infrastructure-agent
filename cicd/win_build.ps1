...
if (-Not $skipSigning) {
    iex "& $signtool sign /d 'New Relic Infrastructure Agent' /n 'New Relic, Inc.'  .\external_content\windows\$arch\winpkg\nr-winpkg.exe"
}
...
If ($tarball) {
    ...
    .\win_tar.ps1 -major $major -minor $minor -patch $patch -arch $arch
}
If (-Not $installer) {
    exit 0
}
